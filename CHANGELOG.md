...
## [0.2.17] - 2026-09-26（终局审计：E2E 错误分支 + config 持久化测试）

### E2E（错误分支真实浏览器验证）

- **新增「复盘跳步失败显示可见错误」E2E**：mock review_step reject，真实 Chromium 断言 review-error 可见（单测之外的真实浏览器覆盖）

### Rust（config 持久化测试）

- **config.rs 新增 3 个单测**：缺文件创建默认 / save-load 往返保留（depth/time/threads）/ 坏 JSON 备份 .bak + 默认重建
- 测试用 AtomicU64 唯一临时目录（防并行碰撞）；修复调试误删 depth 断言（反向审查确认）

### 验证

- 前端 27 单测 + E2E 6→7；后端 cargo 24→27 测试全绿

## [0.2.16] - 2026-09-26（终局审计：common.rs 识别转换逻辑测试覆盖）

### Rust（测试覆盖盲区修复）

- **common.rs 新增 5 个单测**：detections_to_board 行列映射 / 阵营判定（中路 k）/ 越界棋子跳过 / 缺棋盘拒绝；detections_bound 裁剪框计算 + 缺棋盘拒绝
- **Detection::new 改 pub(crate)**（测试可用纯构造器，不影响运行时行为）

### 验证

- 后端 cargo 19→24 测试全绿（fmt/clippy 无警告）；前端未改动
...
- **移除「模式」死下拉**：`mode` ref 从未被逻辑读取，切换无行为变化（假功能）——改为静态「连线分析」标签；「连线对战/人机对弈」仍为产品决策项（R8），不再以假下拉误导
- **构建体积优化**：gzip 148.89KB → 130.12KB（移除 NSelect 相关死代码）

### Rust（数据正确性验证）

- **GameHistory 新增 4 个单测**：push 更新 current_fen / 空 FEN 不覆盖 / reset 清空 / 2048 截断保留最新

### 验证

- 前端 27 测试 + build + E2E 6/6；后端 cargo 15→19 测试 全绿

## [0.2.13] - 2026-09-26（终局审计补位：历史操作失败可见反馈 + README 重载提示）

### 前端（A 层补位：同类缺口追查）

- **清空历史/加载历史失败可见反馈**：`clearHistory`/`loadHistory` 失败由仅 console.error 改为内联错误文本（复用 `review-error`），与复盘跳步一致；成功时清空
- **README 快速上手补「监听中修改需点「重载引擎」后生效」**（文档与 v0.2.12 行为一致）

### 测试

- 新增「清空历史失败显示可见错误」回归；前端 26→27 测试

### 验证

- format/lint/typecheck/build/E2E 6/6 全绿；后端 cargo 15/15 无回归

## [0.2.12] - 2026-09-26（终局审计补位：配置保存「需重载」清晰提示）

### 前端（C 层配置生效清晰度 + A 层提示交叉补位）

- **引擎运行中保存配置提示需重载**：`saveEngineConfig` 在 `isEngineRunning` 时不再直接关闭抽屉，改为 `message.info("配置已保存，正在监听中——点「重载引擎」后新参数才生效")`，引导用户完成重载；非运行中仍正常提示+关闭
- **修复伪反馈**：此前保存成功只提示「配置已保存」，用户误以为参数已生效（实际需重载）

### 测试

- 新增「引擎运行中保存配置提示需重载」回归；前端 25→26 测试

### 验证

- format/lint/typecheck/build/E2E 6/6 全绿；后端 cargo 15/15 无回归

## [0.2.11] - 2026-09-26（终局审计补位：复盘跳步失败可见反馈）

### 前端（A 层补位：用户点了没反馈）

- **Analyse 复盘跳步失败可见错误**：`reviewStep` 失败由仅 console.error 改为内联错误文本（`review-error`），成功时清空；复盘状态与用户感知一致
- **回归测试**：新增「复盘跳步失败显示可见错误」用例

### 验证

- 前端 24→25 测试；format/lint/typecheck/build/E2E 6/6 全绿
