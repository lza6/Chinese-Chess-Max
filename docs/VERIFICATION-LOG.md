...
## 2026-09-26 第十一轮（v0.2.12：配置保存需重载清晰提示）
- 引擎运行中保存配置提示需重载（修复「配置已保存但未生效」伪反馈）
- 前端 26 测试 + E2E 6/6 + build 全绿；后端 15/15

## 2026-09-26 第十二轮（v0.2.13：历史操作失败可见反馈 + README 同步）
- clearHistory/loadHistory 失败可见反馈（与复盘跳步一致，同类缺口追查）
- README 快速上手补「监听中修改需重载引擎」说明
- 前端 27 测试 + E2E 6/6 + build 全绿；后端 15/15

## 2026-09-26 第十三轮（v0.2.14：假功能消除 + 数据单测）
- 移除 mode 死下拉（切换无行为变化的假功能）→ 静态标签；gzip 130KB（-18KB）
- GameHistory 4 个单测（push/空 FEN/reset/2048 截断）；后端 19 测试全绿

## 2026-09-26 第十四轮（v0.2.15：死代码清理）
- 删除 reviewTotal 死状态（只写不读）；onReviewState 保留 index/fen
- 前端 27 测试 + E2E 6/6 + build 全绿；后端 19/19

## 2026-09-26 第十五轮（v0.2.16：识别转换逻辑测试覆盖）
- common.rs 5 个单测（行列映射/阵营/越界/裁剪框/缺棋盘）；Detection::new pub(crate)
- 后端 24 测试全绿；前端未改动

## 2026-09-26 第十六轮（v0.2.17：E2E 错误分支 + config 持久化测试）
- E2E +1（复盘跳步失败可见错误，真实浏览器）；config.rs +3（默认创建/往返/坏文件备份）
- 前端 27 + E2E 7 + 后端 27 全绿
